...
export class BlogService {
  ...
  constructor() {
    this.arrPost = 'http://localhost:3000/api/blog'
  ...
  getAll(): Promise<Post[]> {
    return firstValueFrom(this.httpClient.get<Post[]>(this.arrPost));
  }
...
// TODO Añadir sweetalert a create?
  ...
  //   });
  // }
  updateById(id_post: number, postModificado: Post): Promise<Post> {
    return firstValueFrom(
      this.httpClient.put<Post>(
        `${this.arrPost}/editar/${id_post}`,
        postModificado
      )
    );
  }

  deleteById(id_post: number) {
    return firstValueFrom(
      this.httpClient.delete<any>(`${this.arrPost}/${id_post}`)
    );
  }
